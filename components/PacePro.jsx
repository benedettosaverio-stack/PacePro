...
useEffect(() => {
  ...
  // Bottom nav
  const BottomNav = () => (
    <div className='bottom-nav' style={{position:'fixed',bottom:0,left:0,right:0,zIndex:100,background:'rgba(7,8,11,0.96)',backdropFilter:'blur(24px)',borderTop:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'flex-start',paddingTop:6,paddingBottom:'calc(env(safe-area-inset-bottom, 0px) + 4px)'}}>
      {[['home','home','Accueil'],['running','running','Cardio'],['muscu','muscle','Muscu'],['strava','strava','Strava'],['nutrition','nutrition','Nutrition']].map(([t,icon,label])=>{
        ...
        return (
          <button key={t} onClick={()=>setTab(t)} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-start',gap:3,background:'none',border:'none',cursor:'pointer',fontFamily:'DM Mono, monospace',padding:'4px 0 0',position:'relative',minHeight:44}}>
            ...
            <span style={{fontSize:8,fontWeight:700,letterSpacing:'0.06em',color:active?'#FF0040':'rgba(255,255,255,0.18)',textTransform:'uppercase',lineHeight:1}}>{label}</span>
          </button>
        );
      })}
    </div>
