  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', fontFamily: 'Syne, sans-serif' }}>{label}</span>
        <span style={{ fontSize: 13, fontWeight: 800, color: '#fff', fontFamily: 'DM Mono, monospace' }}>{value}{unit}</span>
      </div>
...
function MealCard({ meal, tag, color }) {
  return (
    <div style={{ background: 'rgba(255,255,255,0.05)', border: `1px solid ${color}30`, borderRadius: 16, padding: '14px 16px', marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: '#fff' }}>{meal.name}</div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {[['Kcal', meal.kcal], ['Prot.', `${meal.prot}g`], ['Carbs', `${meal.carbs}g`], ['Lip.', `${meal.fat}g`]].map(([l, v]) => (
          <div key={l} style={{ flex: 1, background: 'rgba(255,255,255,0.04)', borderRadius: 8, padding: '6px 4px', textAlign: 'center' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#fff', fontFamily: 'DM Mono, monospace' }}>{v}</div>
            <div style={{ fontSize: 8, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', marginTop: 1 }}>{l}</div>
          </div>
        ))}
      </div>
    </div>
...
export default function SmartNutritionModule({ onBack }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: '#fff', fontFamily: 'Syne, sans-serif', paddingBottom: 100 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '0 16px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, paddingTop: 16 }}>
          ...
          <button onClick={() => setShowProfileSetup(true)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '6px 12px', fontSize: 11, color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontFamily: 'inherit' }}>
            ⚙️ Profil
          </button>
        </div>
        ...
        {activity && (
          <div style={{ background: `rgba(255,255,255,0.04)`, border: `1px solid ${accentColor}30`, borderRadius: 16, padding: '12px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 2 }}>{activity.name}</div>
        ...
        {/* Conseil IA */}
        <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', border: `1px solid ${accentColor}25`, borderRadius: 20, padding: '18px', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 13, lineHeight: 1.75, color: 'rgba(255,255,255,0.8)', minHeight: 60 }}>
        ...
        {/* Objectif calorique */}
        <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', border: `1px solid rgba(255,255,255,0.08)`, borderRadius: 20, padding: '18px', marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.15em', fontFamily: 'DM Mono, monospace' }}>Objectif du jour</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: accentColor, fontFamily: 'DM Mono, monospace' }}>{baseKcal} <span style={{ fontSize: 11, fontWeight: 400, color: 'rgba(255,255,255,0.4)' }}>kcal</span></div>
          </div>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.15em', fontFamily: 'DM Mono, monospace', marginBottom: 12 }}>Suggestions de repas</div>
      ...
      {showProfileSetup && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setShowProfileSetup(false)}>
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 480, background: '#13161f', borderRadius: '24px 24px 0 0', padding: '20px 20px 48px' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block', marginBottom: 8 }}>Poids (kg)</label>
              <input type="number" value={weight} onChange={e => setWeight(+e.target.value)} style={{ width: '100%', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '12px 14px', color: '#fff', fontSize: 16, fontFamily: 'DM Mono, monospace', outline: 'none' }} />
            </div>
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block', marginBottom: 8 }}>Objectif</label>
